...
def test_language_models_results_paths() -> None:
    ...
    assert isinstance(language_models_results_paths.tfidf_embeddings_frame_parquet, Path)
    assert isinstance(language_models_results_paths.tfidf_cosine_similarities_parquet, Path)
    assert isinstance(language_models_results_paths.word2vec_embeddings_frame_parquet, Path)
    assert isinstance(language_models_results_paths.word2vec_cosine_similarities_parquet, Path)
    assert isinstance(language_models_results_paths.glove_embeddings_frame_parquet, Path)
